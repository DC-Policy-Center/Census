...
def pull_variable_data(yr,state,var,key):
    ...
    base_csa = "https://api.census.gov/data/%s/acs5?get=NAME,%s&for=metropolitan statistical area/micropolitan statistical area:*&in=state:%s&key=%s"%(str(yr),str(var),str(state),str(key))
...
for j in range(len(var_list)):
    ...
    pulled_df = pull_variable_data(2015,'24',var,census_key)
